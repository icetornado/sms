...
<div id="tabs" class="section-container auto" data-section style="padding:63px 22px 22px 22px;">
        ...
        <div style="width:100%;text-align:center;" class="row">
            <?php //foreach($data as $level => $d): ?>
                <a style="font-weight:bold;font-size:18px;text-transform:uppercase;color:#edf7f9;background:rgba(199,227,242,0.35) url(img/smst_hof_bg.png) repeat-y scroll left top;background-size:50% 1px;width:50%;float:left;padding:14px;" href="#level-<?php echo $level; ?>">Level <?php echo $level; ?></a>
            <?php //endforeach; ?> 
        </div>
        
        <?php foreach($data as $level => $d): ?>
            <section id="level-<?php echo $level; ?>">
                <p class="title" style="font-weight:bold;font-size:18px;text-transform:uppercase;color:#edf7f9;background:rgba(199,227,242,0.35) url(img/smst_hof_bg.png) repeat-y scroll left top;background-size:50% 1px;width:50%;float:left;padding:14px;"><a href="#level-<?php echo $level; ?>">Level <?php echo $level; ?></a></p>
                <?php if(isset($d['boss'])): ?>
                <div>Bosses</div>
                <?php $bosses = $d['boss']; ?>
